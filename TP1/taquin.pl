...
%   heuristique2(U, H).  % ensuite utilisez plutot l'heuristique 2  
   ...
   */
   ...
   heuristique1(U, H) :-
    	findall(X, (piece_placee(U, X),X\=vide), Liste_placees), 
    	length(Liste_placees, Nb), 
    	H is (8 - Nb).
   ...
   % entre sa position courante et sa positon dans l'etat final
    ...
    heuristique2(U, H) :-
      final_state(Fin),
      findall(Elt,(coordonnees([L,C], U, Elt), coordonnees([Lf,Cf], Fin, Elt), Elt \= vide, [L, C] \= [Lf, Cf]), List_mal_placees),
      findall(Dist, (member(X,List_mal_placees), distance_man(U, Fin, X, Dist)), List_dist),
      sumlist(List_dist, H).
